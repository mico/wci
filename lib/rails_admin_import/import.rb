    ...
    module ClassMethods
      ...
      def rails_admin_import(opts)
        ...
        case opts[:format]
          ...
          when :ical
            return ical_import(temp_file, opts[:lookup], opts[:role], opts[:user])
          else
      ...
      def ical_import(temp_file, lookup_field_name, role, current_user)
        ...
        ical.events.each do |entry|
          ...
          if ImportedEvent.where(ical_uid: entry.uid.to_s).empty?
            ...
            # have to add ical_uid field to model
            object = self.import_initialize(new_attrs, :ical_uid, new_attrs[:ical_uid])


            #object.ical_uid = new_attrs[:uid]

            # create location

            skip_nested_save = true
            ...
            if object.errors.empty?
              if skip_nested_save
                ...
                results[:success] << "Skipped nested save: #{object.ical_uid} #{object.send(label_method)}"
                results[:confirmation] << object
              elsif object.save
